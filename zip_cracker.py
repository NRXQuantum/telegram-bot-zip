#!/usr/bin/env python3
"""
Zip Cracker with Telegram Bot - Multi‑format support (ZIP, RAR, 7z, PDF)
Coded by rebnX (Enhanced & Extended)
"""
...
def print_usage():
    ...
    print(f"[*] Usage 1 (Default): python {prog} YourFile.ext")
    print("         └─ Tries 'password_list.txt' only (no numeric fallback)")
    print(f"[*] Usage 2 (Custom Dict): python {prog} YourFile.ext YourDict.txt")
    print(f"[*] Usage 3 (Dict Directory): python {prog} YourFile.ext YourDictDirectory")
    print("\n--- Mask Attack ---")
    print(f"[*] Usage 4 (Mask): python {prog} YourFile.ext -m 'your?dmask?l'")
    print("[*]  ?d: digits, ?l: lowercase, ?u: uppercase, ?s: symbols, ??: literal '?'")
    print("\n--- Numeric Attack (optional) ---")
    print(f"[*] Usage 5: python {prog} YourFile.ext --numeric")
    print("         └─ After dictionary, tries 1-6 digit numbers")
    print("\n--- Telegram Bot Mode ---")
    print(f"[*] Usage 6 (Bot): python {prog} --bot")
    print("    Set environment variable TELEGRAM_BOT_TOKEN or hardcode token below.")
