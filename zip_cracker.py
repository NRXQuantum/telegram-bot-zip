...
Auto‑cleanup of temporary files after 1 hour of inactivity
...
def update_activity():
    """Call this whenever a new file is uploaded or a new job starts."""
    global last_activity_time
    with cleanup_lock:
        last_activity_time = time.time()

def cleanup_old_temp_dirs():
    """Delete temp directories and leftover files older than 2 hours."""
    now = time.time()
    # Clean temp_* folders in current directory (bot)
    for item in os.listdir('.'):
        if item.startswith('temp_') and os.path.isdir(item):
            try:
                mtime = os.path.getmtime(item)
                if now - mtime > 7200:  # 2 hours old
                    shutil.rmtree(item)
            except Exception:
                pass
    # Clean web_job_* folders in system temp
    sys_temp = tempfile.gettempdir()
    for item in os.listdir(sys_temp):
        if item.startswith('web_job_') and os.path.isdir(os.path.join(sys_temp, item)):
            ...
            try:
                ...
                pass
    # Remove any *.fixed.tmp files in current dir
    for f in os.listdir('.'):
...
def cleanup_worker():
    """Background thread that checks idle time and cleans up if needed."""
    global last_activity_time
    while True:
...
def main():
    try:
        ...
        # Start cleanup thread (if any service is running)
